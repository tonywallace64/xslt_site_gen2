...
-spec interval_iterator(
		...
		Tree::gb_tree()) -> gb_trees:iter().
interval_iterator(_,_,{0,nil}) -> [];
interval_iterator(MinB={MinValue,_},MaxB={MaxValue,_},{_,Iterator}) when MinValue =< MaxValue ->
  gb_trees:iterator({1,ii1(MinB,MaxB,Iterator)}).

ii1(_,_,nil) -> nil;
ii1(MinB,MaxB,{Key,Value,Smaller,Bigger}) ->
  case value_test(MinB,MaxB,Key) of
		too_small -> ii1(MinB,MaxB,Bigger);
		too_large -> ii1(MinB,MaxB,Smaller);
		in_range ->
			{Key,Value,check_for_too_small(MinB,Smaller),check_for_too_large(MaxB,Bigger)}
	end.

check_for_too_small(_,nil) -> nil;
check_for_too_small(MinB,{Key,Value,Smaller,Bigger}) ->
  case value_test_min(MinB,Key) of
		too_small -> check_for_too_small(MinB,Bigger);
		big_enough -> {Key,Value,check_for_too_small(MinB,Smaller),Bigger}
  end.

check_for_too_large(_,nil) -> nil;
check_for_too_large(MaxB,{Key,Value,Smaller,Bigger}) ->
  case value_test_max(MaxB,Key) of
		too_large -> check_for_too_large(MaxB,Smaller);
		small_enough -> {Key,Value,Smaller,check_for_too_large(MaxB,Bigger)}
  end.
...
ii_comparator_test() ->
  ...
  {'EXIT',{_,_}} = (catch gb_trees_tools:value_test_max({1,not_a_boolean},1)),
  ok.

ii_test() ->
  ok = ii_comparator_test(),
  E = gb_trees:empty(),
  [] = interval_iterator({0,true},{10,true},E),
  Five = gb_trees:insert(5,five,E),
	%% the following iterator should fail as MinBoundary > MaxBoundary
	{'EXIT',{function_clause,_}} = (catch interval_iterator({10,true},{0,true},Five)),
  FiveItr = interval_iterator({0,true},{10,false},Five),
  {5,five,[]} = gb_trees:next(FiveItr),
  OddTree = 
		lists:foldl(
			fun ({Key,Value},Acc) -> gb_trees:insert(Key,Value,Acc) end,
			Five,
			[{1,one},{3,three},{9,nine},{7,seven}]),
  ThreeToSevenItr = interval_iterator({2,true},{8,false},OddTree),
  {3,three,FiveToSevenItr} = gb_trees:next(ThreeToSevenItr),
...
ii_random_test2(MaxVal,NumTests) ->
  ...
  io:format("I1=~p~nI2=~p~n",[I1,I2]),
	ii_random_test3({I1,MinIncl},{I2,MaxIncl},MaxVal,SetSize),
	ii_random_test2(MaxVal,(NumTests-1)).

ii_random_test3(P1={I1,_},P2={I2,_},MaxData,SetSize) ->
  {MinB,MaxB} =
    case I1 < I2 of
		  true -> {P1,P2};
			false -> {P2,P1}
  ...
  io:format("ii_random_test3: main code~n",[]),
	io:format("MinB=~p~nMaxB=~p~nMaxData=~p~nSetSize=~p~n",[MinB,MaxB,MaxData,SetSize]),
  {MinVal,_}=MinB,
  {MaxVal,_}=MaxB,
  true = (MinVal =< MaxVal),

  DataSet = make_set(MaxData,SetSize),
	DataTree = lists:foldl(fun (X,Acc) -> gb_trees:enter(X,X,Acc) end, gb_trees:empty(), DataSet), 
  ExpectedResultsTree = 
		lists:foldl(
			fun (X,Acc) -> 
			...
			gb_trees:empty(),
			DataSet),
	TestIterator =  interval_iterator(MinB,MaxB,DataTree),
	RefIterator = gb_trees:iterator(ExpectedResultsTree),
  io:format(
		"~nDataSet=~p~nDataTree=~p~nTestIterator=~p~nRefIterator=~p~n",
